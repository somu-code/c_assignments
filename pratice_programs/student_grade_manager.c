...
void calculate_average(struct student s[]) {
    for (int i = 0; i < struct_student_array_length; i++) {
        s[i].average_marks =
            (s[i].marks.data_structures_and_algorithms +
             s[i].marks.operating_systems +
             s[i].marks.database_management_systems +
             s[i].marks.object_oriented_programming +
             s[i].marks.artificial_intelligence + s[i].marks.machine_learning) /
            6;
    }
}
